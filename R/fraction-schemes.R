...
#' `gravel_sand_mud` is an independent GRADISTAT-compatible
#' gravel/sand/mud scheme with boundaries at 2000 and 63 micrometres.
#' `wentworth_major` is a strict Wentworth / phi-scale major scheme with
#' boundaries at 2000 and 62.5 micrometres, so `gravel_sand_mud` and
#' `wentworth_major` are not guaranteed to produce identical output.
#'
...
gs_fraction_schemes <- function() {
  ...
  gravel_sand_mud <- fraction_rows("gravel_sand_mud", c("gravel", "sand", "mud"), c(2000, 63, 0), c(Inf, 2000, 63))
  rows <- list(
    wentworth_major,
    gravel_sand_mud,
    fraction_rows(
